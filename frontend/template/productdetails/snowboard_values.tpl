...
<div class="adp-specs adp-specs--dimensions">
    ...
    <div class="adp-board">
        <svg class="adp-board__svg" viewBox="0 0 600 220" xmlns="http://www.w3.org/2000/svg" role="img"
             aria-label="Nose {$adpSpecsBoard.nose.value} mm, Waist {$adpSpecsBoard.waist.value} mm, Tail {$adpSpecsBoard.tail.value} mm">
            ...
            {foreach ['nose', 'waist', 'tail'] as $part}
                ...
                <text class="adp-board__label" x="{$m.x}" y="200" text-anchor="middle">{$adpSpecsDimensions.$part.label|escape:'html'} {$m.value} mm</text>
            {/foreach}
        </svg>
    </div>
    {/if}
